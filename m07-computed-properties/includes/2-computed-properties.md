...
We currently have our application interface working so that clicking any of the other three thumbnails will switch the larger image to a view of the selected **feature** that can be added on to the trip package. When each of these features is highlighted, the information on the right side of the page should describe that trip "add-on" feature instead of the general cruise information shown in the image below.
...
Now the new `<h3>` tag should be displayed if you view your **index.html** page in a browser and then click any of the 3 feature images to the right of the main thumbnail image. The problem is that we still have information for the main cruise displayed below the new header, so let's fix clear out that area and use that space for feature information.
...
We are going to display a basic heading for **Cost and Availability** information, so we use a basic `<h3>` tag for that text. This will be followed by two separate pieces of information that are displayed based on the value of computed properties.

- A text string that concatenates simple text with the values of two existing properties to let users know the cost of that feature and the number of tickets remaining
- An HTML object that is based on the number of tickets available in order to compute whether that feature is currently available for purchase.

In your **main.js** file, make sure the `featureQty` is set to `0` for the `featureAlienShip` method. Then add two `computed` properties below the comment that reads `//TODO: Add computed properties`.

- The first computed property involves a simple concatenation of text with two existing variable values to create a full sentence that can be rendered by calling the `{{ featureDetails }}` computed property.
- The second computed property calculates and sets `featureAvailable` to `false` if the current value of `featureQty` is `0`.
...
- Create an `<h4>` tag to display the computed property named `featureDetails`
- Display two `<p>` tags
  - Display "Tickets Available" when `featureAvailable` is true
  - Display "Tickets Sold Out!" when `featureAvailable` is false
...
<div v-show="bookingType=='feature'">
  ...
  <p v-if="featureAvailable=='true'"; style="color:green;">Tickets available</p>
...
- Asteroid Fireworks Display description, price and tickets remaining when thumbnail #2 is clicked
  - "Tickets available" is displayed because `featureAvailability` computed value is `true`
...
- **Alien Spaceship Ride-a-long** description, price and tickets remaining when thumbnail #4 is clicked
  - "Tickets Sold Out!" is displayed because `featureAvailability` computed value is `false`